...
class CreateActivity : Fragment() {
    ...
    private fun REPSwitchListener(toggle: ToggleButton, index: Int){
        if (index == 1){
            ...
            REPtoggles[index].backgroundTintList =
                ColorStateList.valueOf(resources.getColor(R.color.toggle_selected))
            ...
            if (checked){
                ...
                for (dayToggle in REPtoggles){
                    if (dayToggle != button){
                        ...
                        dayToggle.backgroundTintList =
                            ColorStateList.valueOf(resources.getColor(R.color.toggle_unselected))
                    }
                }
                repeatFrequency = REPEnums[index]
            } else if (index == 0){
